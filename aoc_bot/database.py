...
class DatabaseManager:
    ...
    async def _create_schema(self) -> None:
        ...
        CREATE TABLE IF NOT EXISTS chat_configs (
            ...
            UNIQUE(chat_id, leaderboard_id, year)
        );
        ...
        CREATE INDEX IF NOT EXISTS idx_chat_leaderboard ON chat_configs(chat_id, leaderboard_id);
        """
    ...
    async def add_config(
        ...
        Raises:
            ValueError: If configuration already exists.
        """
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                raise ValueError(
                    f"Configuration already exists for chat {chat_id}, "
                    f"leaderboard {leaderboard_id}, year {year}"
                )
            raise
        except Exception as e:
    ...
    async def remove_config(self, chat_id: str, leaderboard_id: str, year: int) -> None:
        ...
        try:
            cursor = await self.conn.execute(
                "DELETE FROM chat_configs WHERE chat_id=? AND leaderboard_id=? AND year=?",
            ...
            if cursor.rowcount == 0:
                logger.warning(
                    f"Config not found for deletion: "
                    f"chat={chat_id}, leaderboard={leaderboard_id}, year={year}"
                )
            else:
                logger.info(
                    f"Removed config: chat={chat_id}, "
                    f"leaderboard={leaderboard_id}, year={year}"
                )
        except Exception as e:
